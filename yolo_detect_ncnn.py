...
from ProgramManager.config import labels,num_classes,bbox_colors,LABEL_TO_CATEGORY,SERVO_INDEX_TO_CATEGORY,SERVO_LABEL,SERVO_COLOR,input_size,conf_thresh,nms_thresh,min_frames,gap_limit

# ── CLI args ────────────────────────────────────────────────────────────────── OK
parser = argparse.ArgumentParser()
parser.add_argument('--model', default='yolo26n', help='Model (yolo11n , yolo26n, yolo11s)')
...
serial = SerialManager(baud=args.baud)
serial.connect()
print("|3| Serial OK")
...
try:
    while True:
        ...
        if collecting:
            ...
            y_off  = 60
            cv2.putText(frame,
                        f"BUFFER: {total} frames  gap:{gap_snap}/{args.gap_limit}",
